...
$sqlStud = "SELECT * FROM student JOIN project_student USING (studID) JOIN mark_student USING (titleID) JOIN lecturer USING (lectID) WHERE lectID='$id' AND posID=1003 AND semester='$rowSem[details]' AND studID='$studID'";
$resultStud = mysqli_query($con, $sqlStud);
$rowStud = mysqli_fetch_assoc($resultStud);
$countStud = mysqli_num_rows($resultStud);

$sqlEV1 = "SELECT * FROM student JOIN project_student USING (studID) JOIN mark_student USING (titleID) JOIN lecturer USING (lectID) WHERE studID='$studID' AND projectID='2001' AND posID=1002";
$resultEV1 = mysqli_query($con, $sqlEV1);
$rowEV1 = mysqli_fetch_assoc($resultEV1);

$sqlEV2 = "SELECT * FROM student JOIN project_student USING (studID) JOIN mark_student USING (titleID) JOIN lecturer USING (lectID) WHERE studID='$studID' AND projectID='2001' AND posID=1004";
...
echo '<b>Supervisor: </b>'.$rowStud['lectName'].'<br>';
...
do { 
  ...
  if ($noRows)
  {
   echo '<tr>';
    ...
    echo '<th width="50" style="text-align:center">'.$row2['percentSV'].'% </th>';
   echo '</tr>';
